...
if(strlen(objnext)<11){
  ...
  for(i=0;objs_id[i]!=NO_ID && i<MAX_IDs_SET;i++){
    if ((objs_loc = game_get_object_location(game,object_get_id(game->object[i]))) != 0){
      strcat(objlocstr,object_get_name(game->object[i]));
      strcat(objlocstr,":");
  ...
  screen_area_puts(ge->help, str);
  sprintf(str, "     Exit or e, Pick or p, Drop or d, Roll or rl");
  screen_area_puts(ge->help, str);
