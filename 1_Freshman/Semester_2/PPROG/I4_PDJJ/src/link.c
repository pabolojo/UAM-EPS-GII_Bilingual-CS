 ...
 */
  Id status;
  /**
 * @brief defines opener with WORD_SIZE + 1 size
 */
  char opener[WORD_SIZE + 1];
};
...
STATUS link_set_opener(Link* link, char* name) {
  if (!link || !name) {
    return ERROR;
  }

  if (!strcpy(link->opener, name)) {
    return ERROR;
  }
...
const char * link_get_opener(Link* link) {
  if (!link) {
    return NULL;
  }
  return link->opener;
...
STATUS link_print(Link* link) {
  ...
  fprintf(stdout, "--> Link (Id: %ld; Name: %s, Opener: %s)\n", link->id, link->name, link->opener);
/*space1*/
