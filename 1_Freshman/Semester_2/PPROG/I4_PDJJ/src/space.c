...
STATUS space_set_ldescription(Space *space, char* desc){
  if (!space || !desc) return ERROR;
  if (!strcpy(space->ldescription, desc)) return ERROR;
  return OK;
}
...
/*WEST*/
  ...
  if (idaux) {
    fprintf(stdout, "---> Upper space %ld.\n",idaux);
  } else {
    fprintf(stdout, "---> No upper space.\n");
  }

  idaux = space_get_down(space);
  if (idaux) {
    fprintf(stdout, "---> Lower space %ld.\n",idaux);
  } else {
    fprintf(stdout, "---> No lower space.\n");
  }
