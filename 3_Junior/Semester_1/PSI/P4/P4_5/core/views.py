...
def applygroup(request):
    ...
    Return: Void
    '''
    context_dict = {"title": "Apply Group"}
...
def convalidation(request):
    ...
    Output: Render object to build the template
    '''
    context_dict = {"title": "Convalidation"}
    student = Student.objects.get(user_ptr_id=request.user.id)
...
def breakpair(request):
    '''
    Method: breakpair
    Description: View function that allows the registered user to break a pair
    Input: Request object with the data of the user
    Output: Render object to build the template
    '''
    context_dict = {"title": "Break Pair"}
    user = Student.objects.get(user_ptr_id=request.user.id)
    context_dict['breakpair_form'] = BreakPairForm(user=request.user)
    context_dict['bad_data'] = False
    context_dict['pair_request'] = False
    context_dict['pair_delete'] = False

    # We check if the form has been filled succesfully
    if request.method == 'POST':
        form = BreakPairForm(user=user, data=request.POST)
        if form.is_valid():

            pair = Pair.objects.get(
                id=request.POST.get('pair'))

            # We check if the pair is validated
            if (pair.validated is False):
                #  If is not, we delete the pair
                pair.delete()
                context_dict['pair_delete'] = True
            # If is validated and has not been requested to be broken
            # we set the request and save the object
            elif(pair.studentBreakRequest is None):
                pair.studentBreakRequest = user
                pair.save()
                context_dict['pair_request'] = True
            # If it has already a break request we delete the pair
            else:
                pair.delete()
                context_dict['pair_delete'] = True
        ...
        # If the data is not correct
            ...
            context_dict['bad_data'] = True

    return render(request, 'breakpair.html', context=context_dict)
