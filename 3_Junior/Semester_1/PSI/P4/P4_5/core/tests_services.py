...
LANDING_TITLE = r"EDAT - Home"  # redirect here after login
...
PAIR_SAME_USER = 'You have requested to be paired with'
PAIR_FORM_LABEL = 'student2'
PAIR_SELECTION_ERROR = 'The data you send is incorrect'
...
SERVICE_DEF = {LOGIN_SERVICE: {
    ...
    CONVALIDATION_SERVICE: {
        ...
        "pattern": r"You satisfy the requirements"
},
    PAIR_SERVICE: {
        "title": PAIR_TITLE,
...
class LogInOutServiceTests(ServiceBaseTest):
    ...
    def test02_log_in(self):
        ...
        self.client1.post(reverse(LOGIN_SERVICE), self.paramsUser1,
                          follow=True)
        # check students ids are
        # correctnt1.session.get(USER_SESSION_ID)),self.user1.id)
