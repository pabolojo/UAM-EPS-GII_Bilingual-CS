...
def processARPRequest(data:bytes,MAC:bytes)->None:
    ...
    if data[24:28] != struct.pack('!I', myIP):
        return

    reply = createARPReply(myIP, MAC)
...
def processARPReply(data:bytes,MAC:bytes)->None:
    ...
    if data[8:14] != MAC or data[24:28] != struct.pack('!I', myIP) or data[18:24] != myMAC:
        return

    with globalLock:
        if requestedIP is not None and data[14:18] != struct.pack('!I', requestedIP):
            return
    ...
    with cacheLock:
        cache[struct.unpack('!I', data[14:18])[0]] = MAC
...
def createARPRequest(ip:int) -> bytes:
    ...
    '''
    global myMAC, myIP
    frame = ARPHeader + bytes.fromhex('0001') + myMAC + struct.pack('!I', myIP) + bytes(6) + struct.pack('!I', ip) + bytes(4)
    return frame
...
def createARPReply(IP:int ,MAC:bytes) -> bytes:
    ...
    '''
    global myMAC,myIP
    frame = ARPHeader + bytes.fromhex('0002') + myMAC + struct.pack('!I', myIP) + MAC + struct.pack('!I', IP) + bytes(4)
    return frame
...
def process_arp_frame(us:ctypes.c_void_p,header:pcap_pkthdr,data:bytes,srcMac:bytes) -> None:
    ...
    if data[6:8] == bytes.fromhex('0001'):
        processARPRequest(data, srcMac)
    elif data[6:8] == bytes.fromhex('0002'):
...
def initARP(interface:str) -> int:
    ...
    arpInitialized = False
    registerCallback(process_arp_frame, 2054)
    myMAC = getHwAddr(interface)
    myIP = getIP(interface)
    if ARPResolution(myIP) is None: # Nadie está suplantando nuestra IP
        arpInitialized = True
        return 0
...
def ARPResolution(ip:int) -> bytes:
    ...
    while counter < 3:
        sendEthernetFrame(arpRequest, len(arpRequest), 2054, broadcastAddr)
        time.sleep(1)
        with globalLock:
