...
def process_Ethernet_frame(us:ctypes.c_void_p,header:pcap_pkthdr,data:bytes) -> None:
    ...
    if ethertype not in upperProtos:
        return
    upperProtos[ethertype](us, header, data[14:], origen)
...
def registerCallback(callback_func: Callable[[ctypes.c_void_p,pcap_pkthdr,bytes],None], ethertype:int) -> None:
    ...
    upperProtos[struct.pack('!H', ethertype)] = callback_func
...
def startEthernetLevel(interface:str) -> int:
    ...
    # Una vez hemos abierto la interfaz para captura y hemos inicializado las variables globales (macAddress, handle y 
    # levelInitialized) arrancamos el hilo de recepción
    recvThread = rxThread()
...
def stopEthernetLevel()->int:
    ...
    '''
    if levelInitialized is False and handle is not None:
        return -1
...
def sendEthernetFrame(data:bytes,len:int,etherType:int,dstMac:bytes) -> int:
    ...
    if not levelInitialized and handle is None:
        return -1
    ...
    extralen = 0
    extra = bytes(0)
    if len < ETH_FRAME_MIN - 14:
    	extra = bytes(ETH_FRAME_MIN - 14 - len) #14 son los bytes que ocupa la cabecera Ethernet
    	extralen = ETH_FRAME_MIN - 14 - len

    trama = dstMac + macAddress + struct.pack('!H', etherType) + data + extra

    if pcap_inject(handle, trama , len + 14 + extralen) != len + 14 + extralen:
        return -1   
    return 0
