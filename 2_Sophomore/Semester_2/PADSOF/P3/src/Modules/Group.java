...
/**
 * This Group class contains all the esential information of groups
 *
 *
 */
public class Group implements Comparable, Serializable {
	...
	private Set<Registered> registered;
	...
	public Group(String name, String description, Registered owner, Group father) {
		...
		this.registered = new TreeSet<Registered>();
		this.subGroups = new TreeSet<Group>();
	...
	public boolean joinGroup(Registered r) {
		...
		if (r.isBanned() || this.registered.contains(r)) {
			app.getCurrentUser().sendNotification(new Notification("Error joining Group"));
			return false;
		}

		if (checkTree(r, getRoot())) {
			app.getCurrentUser().sendNotification(new Notification("You already belong to a group of the same kind"));
			return false;
		}
		this.registered.add(r);
		app.getCurrentUser().sendNotification(new Notification("You have joined " + this.name));
	...
	public boolean leaveGroup(Registered r) {
		Application app = Application.getApplication();
		if (r.isBanned() || !this.registered.contains(r)) {
			app.getCurrentUser().sendNotification(new Notification("You can NOT leave the group"));
			return false;
		}

		this.registered.remove(r);
		app.getCurrentUser().sendNotification(new Notification("You have left group " + this.name));
		return true;
	...
	public static boolean getAffinityReport(Group g1, Group g2) {
		...
		if (!g1.getUsers().contains(user) || !g2.getUsers().contains(user)) {
			user.sendNotification(
					new Notification("You can NOT ask for a affinity report if you do NOT belong to both groups"));
			return false;
		}
		...
		user.sendNotification(new Notification(
