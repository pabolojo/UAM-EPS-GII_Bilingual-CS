...
public abstract class Project implements Comparable, Serializable, GrantRequest {
	...
	public boolean subscribe(Registered r) {
		if (r.isBanned()) {
			r.sendNotification(new Notification("You can NOT subscribe if you are banned"));
			return false;
		}
		if (!this.voteUsers.contains(r)) {
			r.sendNotification(new Notification("You can NOT subscribe if you have NOT voted"));
			return false;
		}
		r.sendNotification(new Notification("You have subscribed to project " + this.name));
	...
	public boolean vote(Registered r) {
		Application app = Application.getApplication();
		if (r.isBanned()) {
			r.sendNotification(new Notification("You can NOT vote when you are banned"));
			return false;
		}
		if (this.voteUsers.contains(r)) {
			r.sendNotification(new Notification("You already voted to project " + this.name));
			return false;
		}
		this.numVotes++;
		...
		if (app.getThreshold() == this.numVotes) {
			...
			for (Registered re : this.subUsers) {
				re.sendNotification(new Notification("Project " + this.name + " is now fundable!"));
			}
		}
		r.sendNotification(new Notification("You have voted project " + this.name));
		this.date = ModifiableDate.getModifiableDate();
	...
	public boolean expiredProject() {
		if (ModifiableDate.getModifiableDate().minusDays(30).isAfter(this.date)) {
			Application app = Application.getApplication();
			app.getExpiredProjects().add(this);
			this.expired = true;
			this.fundable = false;
			for (Registered r : this.subUsers) {
				r.sendNotification(new Notification("Project " + this.name + " has expired"));
			}
			return true;
		}
		return false;
	...
	public boolean getPopularityReport() {
		...
		if (user.isBanned()) {
			user.sendNotification(new Notification("You can NOT ask for reports while banned"));
			return false;
		}
		if (!this.voteUsers.contains(user)) {
			user.sendNotification(new Notification("You can NOT ask for a popularity report if you have NOT voted"));
			return false;
		}
		user.sendNotification(new Notification("Project: " + this.name + " has " + this.numVotes + " votes."));
		return true;
	}
	...
	public boolean setGrantedBudget(double granted) {
		...
		if (granted == 0) {
			for (Registered r : this.subUsers) {
				r.sendNotification(new Notification("Project " + this.name + " has been rejected by the Council Hall"));
			}
			return false;
		}
		for (Registered r : this.subUsers) {
			r.sendNotification(new Notification(
